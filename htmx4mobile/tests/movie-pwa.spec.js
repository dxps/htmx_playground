...
test("exposes installable PWA metadata and registers a service worker", async ({ page }) => {
  ...
  const registration = await page.evaluate(() => navigator.serviceWorker.ready.then((sw) => sw.active?.state));
  expect(registration).toBe("activated");
});

test("loads cached shell and movie data while offline", async ({ browser }) => {
